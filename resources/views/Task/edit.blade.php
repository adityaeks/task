    ...
    <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200/60 dark:border-zinc-800/60 shadow-sm overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="user" required
                            ...
                            <option value="Ryan" @selected(old('user', $task->user) === 'Ryan')>Ryan</option>
                ...
                {{-- Category --}}
                <div>
                    <label class="block text-xs font-bold text-slate-500 dark:text-zinc-400 mb-1.5">Category <span class="text-rose-500">*</span></label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <select name="category" required
                                class="w-full pl-10 pr-4 py-2.5 text-xs rounded-xl bg-slate-100 dark:bg-zinc-800 border-0 focus:ring-2 focus:ring-indigo-500/50 focus:outline-none @error('category') ring-2 ring-rose-500 @enderror">
                            <option value="Development" @selected(old('category', $task->category) === 'Development')>Development</option>
                            <option value="Live" @selected(old('category', $task->category) === 'Live')>Live</option>
                        </select>
                    </div>
                    @error('category')<p class="text-[11px] text-rose-500 mt-1">{{ $message }}</p>@enderror
                </div>
